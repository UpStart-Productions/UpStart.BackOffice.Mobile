...
- **`date-scroller`** — reworked into a Harvest-style Monday-Sunday week strip on the Time tab (not the original's infinite horizontal scroll), reimplemented with native `Date`/`Intl` instead of moment.js. Each day shows its logged-time total; a date pill + calendar icon + chevrons above the strip page between weeks (tap the pill to jump to today, tap the calendar icon for a full date picker to jump to any week, swipe or use the chevrons to move a week at a time). A "Week total" line sits below — no "Submit" button, since there's no timesheet-approval workflow in the backend to submit to.

Skipped as not applicable here: `avatar-picker`, `feelings`, `video-header`, `gamification` (consumer/wellness-app specific), `calendar`/`weekday-picker`/`color-picker` (no matching feature, and the calendar depends on moment.js), `about-modal` (fully branded content as-is). `progress-line-chart`, `share-sheet`, `rich-text-editor`, and `pdf-export` weren't needed for this pass but are worth a look if the app grows reporting or sharing features.
